...
if getgenv().Memory.IsActive then
    local Memory
    ...
    task.spawn(function()
        while task.wait(1) do
            local minMemory = getgenv().Memory.Params.MinValue
            local maxMemory = getgenv().Memory.Params.MaxValue
...
local function isWithinBoxFOV(position)
    ...
    local fovHeight = getgenv().TriggerBot.Config.BoxFov.Height * 100
    local fovWidth = getgenv().TriggerBot.Config.BoxFov.Width * 100
            ...
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    if onScreen and isWithinBoxFOV(part.Position) then
                        ...
                        if os.clock() - lastClickTime >= getgenv().TriggerBot.Config.TriggerCooldown then  
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode[getgenv().TriggerBot.Config.KeyTrigger.TriggerBot:upper()] then
        isToggled = true
    end
end)
...
local function GetTarget()
    if getgenv().Silent.mode == "target" then
        return TargetPlayer  -- Use the currently targeted player if in 'target' mode
    elseif getgenv().Silent.mode == "normal" then
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if getgenv().CamLock.Normal.Aligner then
            ...
            if getgenv().Silent.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                if getgenv().Silent.mode == "target" then
                    ...
                    Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * getgenv().Silent.Prediction)
                elseif getgenv().Silent.mode == "normal" then
...
local function getDynamicFov(targetPosition)
    ...
    -- Check distance and return corresponding FOV radius from the new structure
    if distance <= getgenv().Ranges.CloseDistance then
        return getgenv().Silent.BoudingSizes.CloseFovRadius
    elseif distance <= getgenv().Ranges.MidDistance then
        return getgenv().Silent.BoudingSizes.MidFovRadius
    elseif distance <= getgenv().Ranges.FarDistance then
        return getgenv().Silent.BoudingSizes.FarFovRadius
    else
        return getgenv().Silent.BoudingSizes.FarFovRadius  -- Default for targets beyond FarDistance
    end
end

-- Function to update the FOV display
local function updateFov(TargetPlayer)
    local settings = getgenv().Silent.BoudingSizes  -- Using the updated Silent structure for FOV sizes
...
local function isPartVisible(part)
    if not getgenv().TargetControl.Safety then 
        return true  
    end
...
local OldPredictionY = getgenv().Silent.Prediction
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        if humanoid then
            ...
            if velocity.Y < -30 and getgenv().TargetControl.Aligner then
                getgenv().Silent.Prediction = 0
                return velocity
            elseif velocity.Magnitude > 50 and getgenv().TargetControl.Aligner then
           
                return humanoid.MoveDirection * 16
            else
         
                getgenv().Silent.Prediction = OldPredictionY
                return velocity
            end
        end
    end
...
local function GetVelocity(player, part)
    if player and player.Character then
        local velocity = player.Character[part].Velocity
        if velocity.Y < -30 and getgenv().Silent.Aligner then
            getgenv().Silent.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and getgenv().Silent.Aligner then
...
local function toggleFeature()
    ...
    local status = getgenv().Silent.Enabled and "Forbidden Enabled" or "Forbidden Disabled"
...
RunService.RenderStepped:Connect(function()
    ...
    if getgenv().Silent.Enabled and IsTargeting then  -- Ensure Silent is enabled and CamLock is engaged
        UpdateFOV()  -- Call UpdateFOV to refresh visibility

        -- Check the mode and act accordingly
        if getgenv().Silent.mode == "target" and TargetPlayer then
